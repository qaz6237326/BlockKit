...
export const parseLexerToken = (
  ...
  switch (token.type) {
    ...
    case "heading": {
      ...
      // Case1: 在列表格式解析的时候, 会存在意外的 heading 格式, 此时作为普通文本处理
      // - 无序列表项1
      //    -
      // 由此, 在这里我们仅处理 # 表示的 heading 格式
    ...
    case "list_item": {
      ...
      tokens.forEach((child, i) => {
        ...
        const level = String(listLevel - 1);
        ...
        if (parent.ordered === false) {
          applyLineMarks(delta, { "list": "bullet", "indent-level": level });
        }
      });
      return delta;
    }
