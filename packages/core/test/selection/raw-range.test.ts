...
describe("selection raw-range", () => {
  ...
  it("is-equal", () => {
    ...
    expect(RawRange.equals(range1, range2)).toBe(true);
    expect(RawRange.equals(range1, range3)).toBe(false);
    expect(RawRange.equals(range3.clone(), range3)).toBe(true);
  });
  ...
  it("intersect", () => {
    const base = RawRange.fromEdge(5, 10);
    expect(RawRange.intersects(base, RawRange.fromEdge(5, 10))).toBe(true);
    expect(RawRange.intersects(base, RawRange.fromEdge(6, 10))).toBe(true);
    expect(RawRange.intersects(base, RawRange.fromEdge(6, 9))).toBe(true);
    expect(RawRange.intersects(base, RawRange.fromEdge(1, 6))).toBe(true);
    expect(RawRange.intersects(base, RawRange.fromEdge(10, 11))).toBe(true);
    expect(RawRange.intersects(base, RawRange.fromEdge(11, 11))).toBe(false);
  });
});
