...
export const PRESET_SHORTCUT: ShortcutFuncMap = {
  [BOLD_KEY]: (event, payload) => {
    if (event[CTRL_KEY] && isKeyCode(event, KEY_CODE.B)) {
      const { editor, keys } = payload;
      const sel = editor.selection.get();
      sel && editor.perform.applyMarks(sel, { [BOLD_KEY]: keys[BOLD_KEY] ? NIL : TRULY });
      return true;
    }
  },
  [ITALIC_KEY]: (event, payload) => {
    if (event[CTRL_KEY] && isKeyCode(event, KEY_CODE.I)) {
      const { editor, keys } = payload;
      const sel = editor.selection.get();
      sel && editor.perform.applyMarks(sel, { [ITALIC_KEY]: keys[ITALIC_KEY] ? NIL : TRULY });
      return true;
    }
  },
};
